...
class CommentServiceTest {
	...
	void getCommentsByPostId_ShouldReturnListOfComments() {
		...
		when(commentRepository.findByPosts_Id(100L)).thenReturn(mockComments);
		...
		verify(commentRepository, times(1)).findByPosts_Id(100L);
	}
	...
	void getCommentsByPostId_ShouldThrowException_WhenNoCommentsExist() {
		...
		when(postRepository.existsByIdAndDeletedIsFalse(100L)).thenReturn(true);
		when(commentRepository.findByPosts_Id(100L)).thenReturn(Collections.emptyList());
