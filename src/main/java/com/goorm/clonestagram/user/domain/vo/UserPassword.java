...
public class UserPassword {
	...
	private void validate(String pw) {
		...
		if (pw.length() > 20)
			throw new BusinessException(ErrorCode.USER_PASSWORD_TOO_LONG);
		if (pw.contains(" "))
			throw new BusinessException(ErrorCode.USER_PASSWORD_SPACE);
		if (!pw.matches(".*[0-9].*"))
			throw new BusinessException(ErrorCode.USER_PASSWORD_NO_NUMBER);
		if (!pw.matches(".*[!@#$%^&*()\\-+].*"))
			throw new BusinessException(ErrorCode.USER_PASSWORD_NO_SPECIAL);
	}
